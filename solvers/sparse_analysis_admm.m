function varargout = sparse_analysis_admm(y, lambda, ...
                                          phi0, psi, D, amplitude, ...
                                          stop_func, ...
                                          varargin)

% sparse_analysis_admm - solve a sparse analysis problem with ADMM
%
%   solve f^* = \Psi x^*
%   where x^* = Argmin_x 1/2 | y - \Phi0 \Psi x |_2^2 + | D^* x |_(1,lambda)
%   using ADMM algorithm where:
%       F(x, y, lambda, u) = F1(x, y) + F2(u, lambda)
%       F1(x, y)           = 1/2 | y - \Phi x |_2^2
%       F2(u, lambda)      = | u |_(1,lambda)
%       G(x, y, u)         = is(DS * x = u)
%
%   x                = sparse_analysis_admm(y, lambda, phi0, psi, D;
%                                           amplitude, stop_func)
%
%   [x, dx_dy_delta] = sparse_analysis_admm(y, lambda, phi0, psi, D;
%                                           amplitude, stop_func,
%                                           'AppliedJacY', delta)
%
%   [x, dx_dlambda]  = sparse_analysis_admm(y, lambda, phi0, psi, D;
%                                           amplitude, stop_func,
%                                           'JacTheta')
%
...
proxG.def = @(xu, y, tau) ...
    funcProxG(xu, tau);
proxG.D{1} =  @(xu, y, tau, dxu) ...
    funcProxG(dxu, tau);
proxG.D{2} =  @(xu, y, tau, dy) ...
    zeros(N, 1);
