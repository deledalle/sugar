function [x, R, info, Ncall, xcall, fcall, gcall] = perform_bfgs(Grad, x, options)
...
%   R is filled using options.repport which takes as input (x,val).
%
%   Copyright (c) 2011 Gabriel Peyre
%   modified 2014 Charles Deledalle

call.alpha = 0.9;
call.N = 0;
call.x = zeros(length(x), 100);
call.f = zeros(1, 100);
call.g = zeros(length(x), 100);

n = length(x);
pars.nvar = n;
pars.fgname = @(x,pars)Grad(x);
options.x0 = x;
options.nvec = getoptions(options, 'bfgs_memory', 20); % BFGS memory
options.maxit = getoptions(options, 'niter', 1000);
options.prtlevel = 0;
options.normtol = getoptions(options, 'normtol', 0.02);
options.tol = getoptions(options, 'tol', eps);
options.verb = 1;

% options.report = @(x,val)struct('E', val, 'timing', cputime()-t);
[x, R, energy, d, H, iter, info, call] = bfgs(pars,call,options);

Ncall = call.N;
xcall = call.x(:, 1:call.N);
...
function [x, R, f, d, H, iter, info, call, X, G, w, fevalrec, xrec, Hrec] ...
    = bfgs(pars, call, options)
...
for run = 1:nstart
    ...
    if nargout > 10
        [x(:,run), R, f(run), d(:,run), HH, iter(run), call, ...
         info(run), X{run}, G{run}, w{run}, ...
         fevalrec{run}, xrec{run}, Hrec{run}] = ...
            bfgs1run(x0(:,run), call, pars, options);
    elseif nargout > 7 % avoid computing fevalrec, xrec, Hrec which are expensive as they grow inside the main loop
        [x(:,run), R, f(run), d(:,run), HH, iter(run), info(run), call, ...
         X{run}, G{run}, w{run}] = bfgs1run(x0(:,run), call, pars, options);
...
if call.N == 0
    call.norm = (1/call.alpha)/norm(x)*norm(g);
    call.dnorm_sc = call.norm./max(abs(g));
    H0 = diag(1./((1/call.alpha)./abs(x).*abs(g))) * call.norm;
    H = H0; % sparse for limited memory BFGS 
end
f = f/call.norm;
g = g/call.norm;
...
nG = 1;
w = 1;
dnorm = norm(g.*call.dnorm_sc, Inf);
...
iter = 1;
while 1
    if (iter > maxit || call.N > maxit) && isfinite(call.f(:,call.N))
        break;
    end
    R(iter) = report(x,f);
    if nvec == 0 % full BFGS
    ...
    if strongwolfe 
	    ...
	    if call.N == 0
                call.dnorm_sc = (1/call.alpha)./max(abs(x));
                call.norm = (1/call.alpha)/norm(x)*norm(g);
	    end
	    f = f/call.norm;
	    g = g/call.norm;
        ...
        [alpha, x, f, g, call, fail, notused, notused2, fevalrecline] = ...
                linesch_ww(x, f, g, p, pars, call, wolfe1, wolfe2, fvalquit, prtlevel);
    end
    ...
    dnorm = norm(d.*call.dnorm_sc, Inf);
    if nargout > 9
        xrec(:,iter) = x;
...
function [xbundle, gbundle, call] = getbundle(x, g, samprad, N, pars, call);
...
for k = 2:N  % note the 2
   ...
   if call.N == 0
       call.dnorm_sc = (1/call.alpha)./max(abs(xpert));
       call.norm = (1/call.alpha)/norm(xpert)*norm(grad);
   end
   f = f/call.norm;
   grad = grad/call.norm;
   call.N = call.N + 1;
...
function [alpha, xalpha, falpha, gradalpha, call, fail, beta, gradbeta, fevalrec] = ...
          linesch_ww(x0, f0, grad0, d, pars, call, c1, c2, fvalquit, prtlevel)
...
while ~done
    ...
    if call.N == 0
        call.dnorm_sc = (1/call.alpha)./max(abs(x));
        call.norm = (1/call.alpha)/norm(x)*norm(grad);
    end
    f = f/call.norm;
    grad = grad/call.norm;
    call.N = call.N + 1;
